...
```{r}
ffx_sub <-  ffx_2018 %>%
  select(assessed_total_value, year_built, land_square_footage, building_square_feet, living_square_feet, bedrooms, half_baths, full_baths)
...
theme_config = list(legend.position = c("bottom"))) 
{
  ...
  output <- ggplot(missing_value, aes_string(x = "feature", 
    y = "num_missing")) + geom_bar(stat = "identity", fill = "#0E879C") + coord_flip() + xlab("Features") +
   ylab("Percent Missing Variables") +
    scale_y_continuous(limits = c(0,200000)) +
    scale_x_discrete(labels=c("half baths", "bedrooms", "full baths", "living sq. ft.", "building sq. ft.", "year built", "land sq. footage", "assessed total value"))
  ...
  plotDataExplorer(plot_obj = output, title = title, ggtheme = ggtheme, theme = theme(axis.title.y = element_text(size = rel(1.5), angle = 90), axis.title.x = element_text(size = rel(1.5)), axis.text = element_text(size = rel(1), colour = "black", face = "bold")))
}
```
...
theme_config = list(legend.position = c("bottom"))) 
{
  ...
  output <- ggplot(missing_value, aes_string(x = "feature", 
   ...
   ylab("Percent Missing Variables") +
    scale_y_continuous(limits = c(0,200000)) +
    scale_x_discrete(labels=c("bedrooms", "bathrooms", "living area", "value total", "year built"))
  geom_label_args_list <- list(mapping = aes(label = paste0(round(100 * 
    pct_missing, 2), "%")))
  output <- output + do.call("geom_label", c(geom_label_args_list, 
     geom_label_args))
...
theme_config = list(legend.position = c("bottom"))) 
{
  ...
  output <- ggplot(missing_value, aes_string(x = "feature", 
   ...
   ylab("Percent Missing Variables") +
    ...
    scale_x_discrete(labels=c("number of half baths", "number of baths", " number of bedrooms", "legal", "year built", "building area (effective)", "building area (gross)", "land", "land acres (acres)" ))
  geom_label_args_list <- list(mapping = aes(label = paste0(round(100 * 
    pct_missing, 2), "%")))
  output <- output + do.call("geom_label", c(geom_label_args_list, 
     geom_label_args))
...
theme_config = list(legend.position = c("bottom"))) 
{
  ...
  output <- ggplot(missing_value, aes_string(x = "feature", 
   ...
   ylab("Percent Missing Variables") +
    scale_y_continuous(limits = c(0,15000))  +
    scale_x_discrete(labels=c("building sq. ft.", "half baths", " bedrooms", "full baths", "living sq. ft.", "year built", "land sq. footage", "assessed total value" ))
  geom_label_args_list <- list(mapping = aes(label = paste0(round(100 * 
    pct_missing, 2), "%")))
  output <- output + do.call("geom_label", c(geom_label_args_list, 
     geom_label_args))
...
plot_1 <- plot_missing_2(ffx_sub, title = "Fairfax County CoreLogic data- 2018", ggtheme = theme_classic(), theme = theme()) 
                         
plot_2 <- plot_missing_2_1(fairfax_sub, title = "Fairfax County data- 2018", ggtheme = theme_classic(), theme = theme())


plot_3 <- plot_missing_3(nk_sub, title = "New Kent County data- 2018", ggtheme = theme_classic(), theme = theme())


plot_4 <- plot_missing_3_1(sub_nk_cl, title = "New Kent County CoreLogic data- 2018", ggtheme = theme_classic(),theme = theme())
